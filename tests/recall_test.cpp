...
TEST_CASE("Recall::load_matrix - file esistente e corretto") {
  std::filesystem::create_directory("test_matrix");
  std::ofstream out("test_matrix/weight_matrix.txt");
  out << "1.0 0.0 -1.0\n";
  out << "0.5 2.0 3.0\n";
  out.close();

  hp::Recall r("test_matrix");
  ...
  std::filesystem::remove_all("test_matrix");
}

TEST_CASE("Recall::load_matrix - file mancante") {
  CHECK_THROWS_AS(hp::Recall("nonexistent"), std::runtime_error);
}

TEST_CASE("Recall::initialize_from_image - immagine valida") {
  std::filesystem::create_directory("test_img");
  sf::Image img;
  ...
  std::filesystem::remove_all("test_img");
  std::filesystem::remove_all("test_matrix");
}
